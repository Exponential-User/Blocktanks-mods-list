...
document.addEventListener('DOMContentLoaded', () => {
	...
	for (let i = 1; i <= 4; i++) {
		setTimeout(() => {
			console.log('%c' + 'WARNING!', 'color: #ff0000; font-size: 72px');
			console.log('%c' + 'Pasting code here may result in your account credentials being stolen.', 'font-size: 20px');
	...
	for (let i = 1; i <= 7; i++) {
		const loadMainMod = setTimeout(() => {
			try {
				...
				console.log('%c' + ' An error has occored! \n', 'color: #ff0000; background-color: #000;', e);
			}
		}, 1000 * i);
...
function resetConsolePlaceholder() {
	...
	if (credentials.controls) {
		...
		chatbox.placeholder = 'Press "t" to talk';
	}
}
...
function enableConsole() {
	if (!credentials.kidMode && !credentials.guest && window.location.hostname != 'simple.blocktanks.net') {
		...
		{
			const minLevel = SERVER_MATCH_DATA.match_chat_minLevel;
			const requireVerifiedEmail = SERVER_MATCH_DATA.match_chat_requireVerifiedEmail;

			const meetsLevelRequirement = (minLevel == undefined || minLevel <= credentials.xpInfo.l);
			const meetsEmailRequirement = (!requireVerifiedEmail || credentials.verified);
			chatEnabled = (meetsLevelRequirement && meetsEmailRequirement);

			if (!chatEnabled) {
				const chatPlaceholder =
					(!meetsLevelRequirement) ? `Reach level ${minLevel} to use the chat` :
					(!meetsEmailRequirement) ? 'Verify your email address to use the chat' :
					'You do not meet the requirements to use the chat';

				chat_type.placeholder = chatPlaceholder;
			}
		}
		chat_type.disabled = !chatEnabled;
...
function showRespawnUI() {
	...
	var countdownInterval = setInterval(function() {
		...
		if (timeLeft <= 0) {
			...
			// TODO: Add auto respawn for VIP, could also have a simple toggle below the button that's stored in local storage
			// - https://discord.com/channels/458034316677611543/1323645611529998336/1323645611529998336
		}
		else {
...
function leaveMatch() {
	confirm('Are you sure you want to leave the match?') && backToTitle();
}
...
function setupFullscreenToggle() {
	...
	const ENABLE_ICON = '/assets/ui/icon_fullscreen.svg';
	const DISABLE_ICON = '/assets/ui/icon_fullscreen_exit.svg';

	// BUG: There's currently no way to determine when a user enters OS level fullscreen with F11
	// We can check for the key F11, however if a user is in fullscreen before they reach the site,
	// it will still be out of sync
	document.addEventListener('fullscreenchange', () => {
		const FULLSCREEN_ENABLED = (document.fullscreenElement != null);
		toggleFullscreenButton.src = (FULLSCREEN_ENABLED) ? DISABLE_ICON : ENABLE_ICON;
	});
}
...
function toggleFullscreen(force = null) {
	...
	const ENABLE_FULLSCREEN = force ?? !FULLSCREEN_ENABLED;
	(ENABLE_FULLSCREEN) ? document.body.requestFullscreen() : document.exitFullscreen();
}
...
function endGameStage1() {
	console.log('Starting end game stage 1');
	showScoreboard(false, true);
	showHelp(false, true);
	...
	if(credentials.guest){
		...
		if (xpInfo.l !== undefined) {
			...
			if (consoleEnabled) enableConsole(); // Update the console with the new XP info
		}

		if (xpInfo.breakdown) {
			console.log('Displaying experience breakdown...');
			displayExperienceBreakdown(xpInfo.breakdown);
		} else {
			// If the breakdown is not available, skip to the second stage.
			console.log('Experience breakdown not available. Skipping to the second stage...');
			endGameStage2();
		}
	}
}
...
function loadAccountInfo() {
	...
	if (credentials.vip) {
		document.getElementById("profile-username").style.backgroundColor = "#ffc94c";
	}
	document.getElementById('profile-username').addEventListener('click', () => {
		let processedDisplayName = credentials.username;
		processedDisplayName = processedDisplayName.replace(/ /g, '%20');
		window.location.href = `user/${processedDisplayName}`;
	});
...
function loadMatchPrompt(yes) {
	...
	document.querySelector('#matchChatSettings').innerHTML = loadInputFields(matchFields.chatSettings);
...
function loadMaps(type, page) {
	...
	if (defaultMaps[type] !== undefined) {
		...
		// BUG: If you switch tabs before this is completed it'll override the maps. We should either ignore or cancel this request with signals.
...
function loadMapsCallback(list, type) {
	...
	for (var i = 0; i < list.length; i++) {
		...
		thumbnailUrl = getCorrectMapThumbnail(map.id, selectedMode, map.version);
		...
		html += `<p style="padding:0px;font-size:14px;">${map.plays?.toLocaleString() ?? 0} Views</p>`;
		if (FLAGS.devTools) {
